...
def main(args):
    ...
    def pde(x, y, v):
        ...
        # add noise tensor torch
        # torch.manual_seed(0)
        # noise = torch.randn_like(dy_t) / 10**3
    ...
    if args.mode == "train":
        ...
        # save
        model.save("saved/ckpts/dr/low_deeponet_dr")
    elif args.mode == "test":
        model.restore(f"saved/ckpts/dr/low_deeponet_dr-{epochs}.pt")
    ...
    # plt.colorbar()
    plt.tight_layout()
    plt.savefig("3diffusion_reaction_low.pdf")
...
def save_lfdata(v_branch, x_trunk, y, y_pred):
    np.savez(f"low_dr_data_train.npz", X0=v_branch[:1000,:], X1=x_trunk[:320,:], y=y.reshape(-1,1)[:320,:], y_low_x=y_pred.T[:320,:])
    np.savez(f"low_dr_data_test.npz", X0=v_branch[-100:,:], X1=x_trunk[-566:,:], y=y.reshape(-1,1)[-566:,:], y_low_x=y_pred.T[-566:,:])
