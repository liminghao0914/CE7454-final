...
def get_data(
    ...
    if residual:
        y_train -= np.squeeze(d["y_low_x"].astype(np.float32))
    # d = np.load(fname_train)
    ...
    if residual:
        y_test -= np.squeeze(d["y_low_x"].astype(np.float32))
...
def main(args):
    fname_train = "test.npz"
    fname_test = "test.npz"
    X_train, y_train, X_test, y_test = get_data(
        fname_train, fname_test, residual=True, stackbranch=True, stacktrunk=True
    )
    ...
    if args.mode == "train":
        ...
        model.save("mf_poisson")
    elif args.mode == "test":
        model.restore(f"mf_poisson-{epochs}.pt")
        ...
        # get from dataset
        d = np.load(fname_test)
        x = np.hstack((d["X1"].astype(np.float32), d["y_low_x"].astype(np.float32)))
        fx = np.hstack((d["X0"].astype(np.float32), d["y_low"].astype(np.float32)))
        residual = np.squeeze(d["y_low_x"].astype(np.float32))
        y_label = d["y"].astype(np.float32)
        # x = []
        # for i in range(m):
        ...
        plt.ylabel("f(x)")
        z = np.zeros_like(x)
        plt.plot(x[:,0], z, "k-", alpha=0.1)
        # Plot source term f(x)
        # for i in range(3,5):
        #     plt.plot(x, fx[i], "--")
        plt.plot(x[:,0], fx[4][:100], "--")
        ...
        plt.plot(x[:,0], y[4][:100], "-")
        plt.xlabel("x")

        # plt.show()
        plt.savefig("mf_poisson.png")
        np.savez_compressed("mf_poisson.npz", x=x, y=y, fx=fx, mae=mae)
